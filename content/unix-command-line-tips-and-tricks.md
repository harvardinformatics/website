...
<pre>${variable%pattern}  Trim the shortest match from the end
...
<pre>dirname=`dirname “$file”`</pre>

or

<pre>dirname=${file%/*}</pre>
...
<pre>i=/n/home_rc/mclamp/myfile.txt
i=`basename $i`
echo $i
> myfile.txt</pre>
...
<pre>touch pog{1..10}
ls pog{1..10}
pog1   pog10  pog2   pog3   pog4   pog5   pog6   pog7   pog8   pog9</pre>
...
<pre>for i in `find . -name “*.fastq” |grep R1 ` ; do    # Note the backticks!
...
<pre>for i in `cat *.dat | awk ‘$1 == “SEQ” { print $5}’` ; do
...
<pre>for i in `cat *.fastq|awk ‘NR%4 == 1’ ` ; do
...
<pre>awk '{ print $3, $4, $10 }' myfile.dat</pre>
...
<pre>awk ' $2 ~ /metal_ion_binding/  {print $0 }' $i > ${i%.*}.metal_ion_binding.dat</pre>
...
<pre>awk 'BEGIN {print "Total"} { s += $4} END {print s}'
Total
471.948
</pre>
...
<pre>awk 'NR % 4 == 2 { s += length($1); t++} END {print s/t}'</pre>
...
<pre>awk 'NR % 4 == 2 { s += length($1); t++} END {print s/t}'  ../pogpipe/testdata/sample_1.fq
36
</pre>
...
<pre>ls -ltra /n/home_rc/mclamp/ > /tmp/dirlist.$(date +%d-%b-%Y).log</pre>

which creates

<pre>/tmp/dirlist.13-Jun-2014.log</pre>
...
<pre>date +%Y-%m-%d\ %R</pre>

gives

<pre>2011-03-25 09:48</pre>

<pre>date +%Y-%m-%d\ %T </pre>

gives

<pre>2011-03-25 09:51:05</pre>
...
<pre>while [ 1 ]; do clear; last |head -10; sleep 3; done</pre>

Keep an eye on who is logged in

<pre>while [ 1 ]; do clear; w; sleep 3; done</pre>
...
<pre>read -p “Enter your name (first last) : “ first name last name
